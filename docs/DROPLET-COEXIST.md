...
GitHub Actions workflow draft: `deploy/ci-deploy.yml` (copy to `.github/workflows/deploy.yml` once the token has `workflow` scope). It builds JDK 21, SCPs the jar, and restarts **only** `optionscanner.service`. Required secrets: `SSH_HOST`, `SSH_USERNAME`, `SSH_PRIVATE_KEY`.
